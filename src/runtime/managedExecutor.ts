    ...
    async execute(input: ManagedExecuteInput): Promise<ExecutionResult> {
      ...
      try {
        outcome = await plugin.execute({
          ...
          authorizedHeadRef: input.permit?.head_ref,
          authorizedHeadCommit: input.permit?.head_commit,
          authorizedResolvedTarget: input.permit?.resolved_target,
          authorizedResolvedDestination: input.permit?.resolved_destination,
