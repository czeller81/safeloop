...
- MCP consequential detection now segments the original tool name before canonicalization lowercases it, so `snake_case`, `camelCase`, `PascalCase`, `kebab-case`, `SCREAMING_SNAKE_CASE`, and dotted/slashed namespaces are all recognized. See [MCP.md](MCP.md) for the classification model and its ASCII-vocabulary limitation.
